<x-app-layout>
    ...
    <div class="bg-[#e4e4e4] shadow-sm rounded-lg overflow-hidden">
        ...
        <div class="p-6 border-b border-[#142143]/20">
            <form method="GET" action="{{ route('guru.index') }}" class="flex flex-col sm:flex-row gap-4">
                <div class="flex-1">
                    <div class="relative">
                        ...
                        <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari nama guru, mata pelajaran, atau unit..." 
                ...
                @if(request('search'))
                    <a href="{{ route('guru.index') }}" class="inline-flex items-center px-3 py-2 border border-gray-300 text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50">
                        Reset
                    </a>
                @endif
            </form>
        ...
        <!-- Import Section -->
        <div class="p-6 border-b border-[#142143]/20">
            <form action="{{ route('guru.import') }}" method="POST" enctype="multipart/form-data" class="flex flex-col sm:flex-row gap-4 items-end">
                @csrf
                <div class="flex-1">
        ...
        <div class="overflow-x-auto">
                ...
                <table class="w-full text-sm text-left text-[#142143]" id="Table">
                    ...
                    <tbody class="divide-y divide-[#142143]/20">
                        ...
                        <tr class="hover:bg-[#142143]/5 transition duration-200">
                            <td class="px-6 py-4 font-medium text-[#142143] text-center">{{ $gurus->firstItem() + $loop->index }}</td>
                            ...
                            <td class="px-6 py-4 flex justify-center">
                                <div class="flex items-center space-x-2">
                                    ...
                                    @csrf
                                     @method('DELETE')
                                        <button type="submit" class="text-[#ffaf00] hover:text-[#142143] p-1 rounded-lg hover:bg-[#ffaf00]/10 transition duration-200">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            @else
                <div class="text-center py-12">
                    ...
                    <h3 class="mt-2 text-sm font-medium text-[#142143]">Tidak ada data guru</h3>
                    @if(request('search'))
                        <p class="mt-1 text-sm text-gray-500">Tidak ditemukan hasil untuk pencarian "<span class="font-medium">{{ request('search') }}</span>"</p>
                        <div class="mt-6">
                            <a href="{{ route('guru.index') }}" class="inline-flex items-center px-4 py-2 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-[#0072BC] hover:bg-[#142143] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#0072BC]">
                ...
                <div class="flex items-center justify-between">
                    <div class="text-sm text-[#142143]">
                        @if(request('search'))
                            ...
                            dari
                            <span class="font-medium">{{ $gurus->total() }}</span> hasil pencarian untuk "<span class="font-medium text-[#0072BC]">{{ request('search') }}</span>"
                        @else
                            Menampilkan
                            <span class="font-medium">{{ $gurus->firstItem() ?? 0 }}</span>
                            sampai
        ...
        <div class="relative p-4 w-full max-w-2xl max-h-full">
            <div class="relative bg-[#e4e4e4] rounded-lg shadow-sm">
                <div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t border-[#142143]/20">
                    ...
                    <button type="button" class="text-[#142143] bg-transparent hover:bg-[#142143]/10 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center" data-modal-hide="tambah-guru-modal">
                ...
                <form action="{{ route('guru.store') }}" method="POST" class="p-4 md:p-5 space-y-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label class="block mb-2 text-sm font-medium text-[#142143]">Nama Guru</label>
                            <input type="text" name="nama" class="bg-white border border-[#142143]/30 text-[#142143] text-sm rounded-lg focus:ring-[#1a5d94] focus:border-[#1a5d94] block w-full p-2.5" placeholder="Masukkan nama guru" required>
                        </div>
                        <div>
                    ...
                    <div class="flex items-center p-4 md:p-5 border-t border-[#142143]/20 rounded-b">
                        ...
                        <button data-modal-hide="tambah-guru-modal" type="button" class="py-2.5 px-5 ms-3 text-sm font-medium text-[#142143] focus:outline-none bg-[#e4e4e4] rounded-lg border border-[#142143]/30 hover:bg-[#142143]/10">Batal</button>
                    </div>
                </form>
            </div>
        </div>
        ...
        <div class="relative p-4 w-full max-w-2xl max-h-full">
            <div class="relative bg-[#e4e4e4] rounded-lg shadow-sm">
                <div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t border-[#142143]/20">
                    ...
                    <button type="button" class="text-[#142143] bg-transparent hover:bg-[#142143]/10 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center" data-modal-hide="edit-guru-modal">
                        <svg class="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 14">
                            <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 1 6 6m0 0 6 6M7 7l6-6M7 7l-6 6"/>
                        </svg>
                    </button>
                ...
                <form id="form-edit-guru" method="POST" class="p-4 md:p-5 space-y-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label class="block mb-2 text-sm font-medium text-[#142143]">Nama Guru</label>
                            <input type="text" name="nama" id="edit-guru-nama" class="bg-white border border-[#142143]/30 text-[#142143] text-sm rounded-lg focus:ring-[#1a5d94] focus:border-[#1a5d94] block w-full p-2.5" required>
                        </div>
                        <div>
                            <label class="block mb-2 text-sm font-medium text-[#142143]">Mata Pelajaran</label>
                    ...
                    <div class="flex items-center p-4 md:p-5 border-t border-[#142143]/20 rounded-b">
                        ...
                        <button data-modal-hide="edit-guru-modal" type="button" class="py-2.5 px-5 ms-3 text-sm font-medium text-[#142143] focus:outline-none bg-[#e4e4e4] rounded-lg border border-[#142143]/30 hover:bg-[#142143]/10">Batal</button>
                    </div>
                </form>
            </div>
        </div>
