<x-app-layout>
    ...
    <div class="bg-[#e4e4e4] shadow-sm rounded-lg overflow-hidden">
        ...
        <div class="p-6 border-b border-[#142143]/20">
            <form method="GET" action="{{ route('kelas.index') }}" class="flex flex-col sm:flex-row gap-4">
                <div class="flex-1">
                    <div class="relative">
                ...
                @if(request('search'))
                    <a href="{{ route('kelas.index') }}" class="inline-flex items-center px-3 py-2 border border-gray-300 text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50">
                        Reset
                    </a>
                @endif
            </form>
        ...
        <div class="overflow-x-auto">
                ...
                <table class="w-full text-sm text-left text-[#142143]" id="Table">
                    ...
                    <tbody class="divide-y divide-[#142143]/20">
                        ...
                        <tr class="hover:bg-[#142143]/5 transition duration-200">
                            <td class="px-6 py-4 font-medium text-[#142143] text-center">{{ $kelas->firstItem() + $loop->index }}</td>
            ...
            @else
                <div class="text-center py-12">
                    ...
                    <h3 class="mt-2 text-sm font-medium text-[#142143]">Tidak ada data kelas</h3>
                    @if(request('search'))
                        <p class="mt-1 text-sm text-gray-500">Tidak ditemukan hasil untuk pencarian "<span class="font-medium">{{ request('search') }}</span>"</p>
                        <div class="mt-6">
                            <a href="{{ route('kelas.index') }}" class="inline-flex items-center px-4 py-2 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-[#0072BC] hover:bg-[#142143] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#0072BC]">
            ...
            <div class="px-6 py-4 border-t border-[#142143]/20">
                <div class="flex items-center justify-between">
                    <div class="text-sm text-[#142143]">
                        @if(request('search'))
                            Menampilkan
                            <span class="font-medium">{{ $kelas->firstItem() ?? 0 }}</span>
                            sampai
                            <span class="font-medium">{{ $kelas->lastItem() ?? 0 }}</span>
                            dari
                            <span class="font-medium">{{ $kelas->total() }}</span> hasil pencarian untuk "<span class="font-medium text-[#0072BC]">{{ request('search') }}</span>"
                        @else
                            Menampilkan
                            <span class="font-medium">{{ $kelas->firstItem() ?? 0 }}</span>
                            sampai
                            <span class="font-medium">{{ $kelas->lastItem() ?? 0 }}</span>
                            dari
                            <span class="font-medium">{{ $kelas->total() }}</span> hasil
                        @endif
                    </div>
                    ...
                    <div class="flex items-center space-x-2">
                        ...
                        {{-- Nomor halaman --}}
                        @php
                            $start = max(1, $kelas->currentPage() - 2);
                            $end = min($kelas->lastPage(), $kelas->currentPage() + 2);
                        @endphp
        ...
        <div class="relative p-4 w-full max-w-md max-h-full">
            <div class="relative bg-[#e4e4e4] rounded-lg shadow-sm">
                ...
                <div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t border-[#142143]/20">
                    ...
                    <button type="button" class="text-[#142143] bg-transparent hover:bg-[#142143]/10 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center" data-modal-hide="tambah-kelas-modal">
                ...
                <form action="{{ route('kelas.store') }}" method="POST" class="p-4 md:p-5 space-y-4">
                    @csrf
                    <div>
                        <label class="block mb-2 text-sm font-medium text-[#142143]">Kelas</label>
                        <input type="text" name="kelas" class="bg-white border border-[#142143]/30 text-[#142143] text-sm rounded-lg focus:ring-[#1a5d94] focus:border-[#1a5d94] block w-full p-2.5" placeholder="Masukkan kelas" required>
                    </div>
                <!-- Modal footer -->
                <div class="flex items-center p-4 md:p-5 border-t border-[#142143]/20 rounded-b">
                    ...
                    <button data-modal-hide="tambah-kelas-modal" type="button" class="py-2.5 px-5 ms-3 text-sm font-medium text-[#142143] focus:outline-none bg-[#e4e4e4] rounded-lg border border-[#142143]/30 hover:bg-[#142143]/10">Batal</button>
                </div>
                </form>
            </div>
        </div>
        ...
        <div class="relative p-4 w-full max-w-md max-h-full">
            <div class="relative bg-[#e4e4e4] rounded-lg shadow-sm">
                ...
                <div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t border-[#142143]/20">
                    ...
                    <button type="button" class="text-[#142143] bg-transparent hover:bg-[#142143]/10 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center" data-modal-hide="edit-kelas-modal">
                        <svg class="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 14">
                            <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 1 6 6m0 0 6 6M7 7l6-6M7 7l-6 6"/>
                        </svg>
                    </button>
                ...
                <form id="form-edit-kelas" method="POST" class="p-4 md:p-5 space-y-4">
                    ...
                    <div>
                        <label class="block mb-2 text-sm font-medium text-[#142143]">Kelas</label>
                        <input type="text" name="kelas" id="edit-kelas" class="bg-white border border-[#142143]/30 text-[#142143] text-sm rounded-lg focus:ring-[#1a5d94] focus:border-[#1a5d94] block w-full p-2.5" required>
                    </div>
                <!-- Modal footer -->
                <div class="flex items-center p-4 md:p-5 border-t border-[#142143]/20 rounded-b">
                    ...
                    <button data-modal-hide="edit-kelas-modal" type="button" class="py-2.5 px-5 ms-3 text-sm font-medium text-[#142143] focus:outline-none bg-[#e4e4e4] rounded-lg border border-[#142143]/30 hover:bg-[#142143]/10">Batal</button>
                </div>
            </form>
        </div>
    </div>
