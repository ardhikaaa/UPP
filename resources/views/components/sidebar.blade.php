...
<div class="lg:hidden fixed top-4 left-4 z-50">
    <button id="sidebar-toggle" type="button" class="p-2 bg-yellow-50 rounded-lg shadow-lg border border-yellow-200 hover:bg-yellow-100 transition-colors focus:outline-none focus:ring-2 focus:ring-yellow-500">
  ...
  <div>
    <div class="mb-8">
      ...
      <div class="flex items-center justify-between mb-4">
        <a href="/kunjungan_uks" class="flex items-center space-x-3">
          <img src="../../../../assets/logo-uks.png" alt="Logo" class="w-10 h-8 md:w-12 md:h-8 lg:w-15 lg:h-10" />
          <h1 class="text-lg font-semibold text-gray-800">UKS</h1>
        </a>
    ...
    <nav class="flex flex-col gap-1">
      <a href="/kunjungan_uks" class="flex items-center gap-3 text-sm font-medium px-3 py-3 rounded-lg transition-colors
        {{ request()->is('kunjungan_uks') ? 'text-yellow-600 bg-yellow-50' : 'text-gray-600 hover:text-yellow-600 hover:bg-gray-50' }}">
        <svg class="w-5 h-5 flex-shrink-0 {{ request()->is('kunjungan_uks*') ? 'text-yellow-500' : 'text-gray-600' }}" 
            xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 24 24">
        <path d="m16 24h-8v-8h-8v-8h8v-8h8v8h8v8h-8z"/>
        </svg>
        <span class="truncate">Data Kunjungan UKS</span>
      </a>

      <!-- Dropdown Menu for Rombel, Siswa, Kelas, Unit -->
      <div class="relative">
        <button type="button" class="dropdown-toggle flex items-center justify-between w-full gap-3 text-sm font-medium px-3 py-3 rounded-lg transition-colors
          ...
          <div class="flex items-center gap-3">
            <svg class="w-5 h-5 flex-shrink-0 {{ request()->is('rombel*') || request()->is('siswa*') || request()->is('kelas*') || request()->is('unit*') ? 'text-yellow-500' : 'text-gray-600' }}" fill="currentColor" viewBox="0 0 24 24">
          ...
          class="dropdown-menu mt-1 ml-6 space-y-1 
          {{ request()->is('rombel*') || request()->is('siswa*') || request()->is('kelas*') || request()->is('unit*') ? '' : 'hidden' }}">
           <a href="/unit" class="flex items-center gap-3 text-sm font-medium px-3 py-2 rounded-lg transition-colors
            ...
            <svg class="w-4 h-4 flex-shrink-0 {{ request()->is('unit*') ? 'text-yellow-500' : 'text-gray-600' }}" fill="currentColor" viewBox="0 0 24 24">
          ...
          <a href="/siswa" class="flex items-center gap-3 text-sm font-medium px-3 py-2 rounded-lg transition-colors
            ...
            <svg class="w-4 h-4 flex-shrink-0 {{ request()->is('siswa*') ? 'text-yellow-500' : 'text-gray-600' }}" fill="currentColor" viewBox="0 0 24 24">
          ...
          <a href="/kelas" class="flex items-center gap-3 text-sm font-medium px-3 py-2 rounded-lg transition-colors
            ...
            <svg class="w-4 h-4 flex-shrink-0 {{ request()->is('kelas*') ? 'text-yellow-500' : 'text-gray-600' }}" fill="currentColor" viewBox="0 0 24 24">
            ...
            <span class="truncate">Kelas</span>
          </a>
          ...
          <a href="/rombel" class="flex items-center gap-3 text-sm font-medium px-3 py-2 rounded-lg transition-colors
            ...
            <svg class="w-4 h-4 flex-shrink-0 {{ request()->is('rombel*') ? 'text-yellow-500' : 'text-gray-600' }}" fill="currentColor" viewBox="0 0 24 24">
            ...
            <span class="truncate">Rombel</span>
          </a>
        </div>
      </div>

       <a href="/guru" class="flex items-center gap-3 text-sm font-medium px-3 py-3 rounded-lg transition-colors
        {{ request()->is('guru') ? 'text-yellow-600 bg-yellow-50' : 'text-gray-600 hover:text-yellow-600 hover:bg-gray-50' }}">
        <svg class="w-5 h-5 flex-shrink-0 {{ request()->is('guru*') ? 'text-yellow-500' : 'text-gray-600' }}" fill="currentColor" viewBox="0 0 24 24">
      ...
      <a href="/obat" class="flex items-center gap-3 text-sm font-medium px-3 py-3 rounded-lg transition-colors
        {{ request()->is('obat') ? 'text-yellow-600 bg-yellow-50' : 'text-gray-600 hover:text-yellow-600 hover:bg-gray-50' }}">
        <svg class="w-5 h-5 flex-shrink-0 {{ request()->is('obat*') ? 'text-yellow-500' : 'text-gray-600' }}" fill="currentColor" viewBox="0 0 24 24">
        ...
        <span class="truncate">Data Obat</span>
      </a>
      ...
      <div class="relative">
        <button type="button" class="dropdown-toggle flex items-center justify-between w-full gap-3 text-sm font-medium px-3 py-3 rounded-lg transition-colors
          ...
          <div class="flex items-center gap-3">
            <svg class="w-5 h-5 flex-shrink-0 {{ request()->is('report/obat*') || request()->is('report/siswa*') || request()->is('report/kunjungan*') || request()->is('report/guru*') ? 'text-yellow-500' : 'text-gray-600' }}" fill="currentColor" viewBox="0 0 24 24">
          ...
          {{ request()->is('report/obat*') || request()->is('report/siswa*') || request()->is('report/kunjungan*') || request()->is('report/guru*') ? '' : 'hidden' }}">
           <a href="/report/kunjungan" class="flex items-center gap-3 text-sm font-medium px-3 py-2 rounded-lg transition-colors
            ...
            <svg class="w-4 h-4 flex-shrink-0 {{ request()->is('report/kunjungan*') ? 'text-yellow-500' : 'text-gray-600' }}" fill="currentColor" viewBox="0 0 24 24">
              <path d="m16 24h-8v-8h-8v-8h8v-8h8v8h8v8h-8z"/>
            </svg>
            <span class="truncate">Laporan Kunjungan</span>
          </a>
          <a href="/report/siswa" class="flex items-center gap-3 text-sm font-medium px-3 py-2 rounded-lg transition-colors
            ...
            <svg class="w-4 h-4 flex-shrink-0 {{ request()->is('report/siswa*') ? 'text-yellow-500' : 'text-gray-600' }}" fill="currentColor" viewBox="0 0 24 24">
              <path d="m22.004,4.498c.001-.865-.525-1.61-1.34-1.898L14.199.319c-1.388-.491-2.916-.492-4.303-.006L3.353,2.603c-.818.286-1.346,1.03-1.346,1.896,0,.867.529,1.611,1.347,1.896l6.507,2.27c.689.24,1.414.361,2.138.361s1.448-.121,2.137-.361l5.864-2.045v4.379c0,.552.448,1,1,1s1-.448,1-1c0,0,.004-6.493.004-6.502Zm-7.209,6.056l3.205-1.118v.564c0,3.309-2.691,6-6,6s-6-2.691-6-6v-.563l3.203,1.118c1.799.627,3.794.627,5.592,0Zm4.205,13.446H5c-.318,0-.617-.151-.805-.407s-.244-.586-.15-.89c1.044-3.358,4.315-5.703,7.955-5.703s6.911,2.345,7.955,5.703c.094.303.039.634-.15.89s-.487.407-.805.407Z"/>
            </svg>
            <span class="truncate">Laporan Siswa</span>
          </a>
          <a href="/report/obat" class="flex items-center gap-3 text-sm font-medium px-3 py-2 rounded-lg transition-colors
            ...
            <svg class="w-4 h-4 flex-shrink-0 {{ request()->is('report/obat*') ? 'text-yellow-500' : 'text-gray-600' }}" fill="currentColor" viewBox="0 0 24 24">
              <path d="m0,11v-5C0,2.692,2.692,0,6,0s6,2.692,6,6v5H0Zm12.258,10.328c-.787-1.075-1.258-2.396-1.258-3.828,0-3.584,2.916-6.5,6.5-6.5,1.432,0,2.752.471,3.828,1.258l-9.069,9.069Zm1.414,1.414c1.075.787,2.396,1.258,3.828,1.258,3.584,0,6.5-2.916,6.5-6.5,0-1.432-.471-2.752-1.258-3.828l-9.069,9.069Zm-4.672-5.242c0-1.655.48-3.194,1.298-4.5H0v5c0,3.308,2.692,6,6,6,1.719,0,3.268-.731,4.363-1.894-.859-1.328-1.363-2.907-1.363-4.606Z"/>
            </svg>
            <span class="truncate">Laporan Obat</span>
          </a>
          <a href="/report/guru" class="flex items-center gap-3 text-sm font-medium px-3 py-2 rounded-lg transition-colors
            ...
            <svg class="w-4 h-4 flex-shrink-0 {{ request()->is('report/guru*') ? 'text-yellow-500' : 'text-gray-600' }}" fill="currentColor" viewBox="0 0 24 24">
              <path d="m14,23c0,.552-.448,1-1,1H1c-.552,0-1-.448-1-1,0-3.866,3.134-7,7-7s7,3.134,7,7ZM7,6c-2.209,0-4,1.791-4,4s1.791,4,4,4,4-1.791,4-4-1.791-4-4-4Zm17-1v8c0,2.761-2.239,5-5,5h-4.526c-.945-1.406-2.275-2.533-3.839-3.227,1.437-1.096,2.365-2.826,2.365-4.773,0-3.314-2.686-6-6-6-1.084,0-2.102.288-2.979.791.112-2.658,2.294-4.791,4.979-4.791h10c2.761,0,5,2.239,5,5Zm-4,10c0-.553-.448-1-1-1h-3.5c-.552,0-1,.447-1,1s.448,1,1,1h3.5c.552,0,1-.447,1-1Z"/>
            </svg>
            <span class="truncate">Laporan Guru</span>
          </a>
        </div>
      </div>
  ...
  <div class="mt-auto">
    ...
    <div class="w-full">
      ...
      <div class="flex flex-col gap-1">
        <!-- Profile Button -->
        <a href="{{ route('profile.edit') }}" class="flex items-center gap-3 text-sm font-medium px-3 py-3 rounded-lg transition-colors text-gray-600 hover:text-yellow-600 hover:bg-gray-50">
          <svg class="w-5 h-5 flex-shrink-0 text-gray-600" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
        ...
        <form method="POST" action="{{ route('logout') }}" class="w-full">
          @csrf
          <button type="submit" class="w-full flex items-center gap-3 text-sm font-medium px-3 py-3 rounded-lg transition-colors text-red-600 hover:text-red-700 hover:bg-red-50">
...
<script>
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    console.log('Sidebar initial state:', {
      hasTranslateXFull: sidebar ? sidebar.classList.contains('-translate-x-full') : 'sidebar not found',
      overlayHidden: overlay ? overlay.classList.contains('hidden') : 'overlay not found'
    });
  });

  // Fallback: Direct function for testing
